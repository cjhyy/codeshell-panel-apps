...
export class MediaLibrary {
  ...
  private thumbnailRequests = new Map<LocalMedia, AbortController>();

  /** One foreground video plus one serial import/frame probe, independent of library size. */
  async withVideoDecoder<T>(
  ...
  async ensureThumbnail(
    ...
    if (existing) {
      const thumbnail = await existing;
      if (thumbnail || !isCurrent() || this.items.get(assetId) !== item) return thumbnail;
      // A redraw can invalidate the first card while a new visible card awaits the same request.
      // Its empty result is not a completed cover for the new card: give that caller its own request.
      if (this.thumbnails.get(assetId) === existing) this.thumbnails.delete(assetId);
      return this.ensureThumbnail(assetId, isCurrent);
    }
    const controller = new AbortController();
    this.thumbnailRequests.set(item, controller);
    ...
    try {
      ...
      if (this.thumbnailRequests.get(item) === controller) this.thumbnailRequests.delete(item);
    }
  }
  ...
  release(item: LocalMedia): void {
    this.thumbnailRequests.get(item)?.abort();
