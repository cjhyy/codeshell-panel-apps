...
2. **写适合本人说出的文案。** 按用户要表达的意思写短句、自然衔接、明确指代，保留事实、语气和必要条件。让每段承担一个叙事作用，并能对应现有可用画面。通过 `set_video_script` 携带 `finish:false` 保存可编辑文稿，不能只在聊天里贴一篇，也不能在画面和字幕尚未完成时提前结束。用户没提供的产品效果、经历和数据不补造；画面不足时调整表达或记录待补内容，不默认安排额外拍摄。
3. **制作可编辑的草稿。** 根据段落安排现有素材的真实源范围，通过 `apply_video_edit` 建立或局部调整序列。先让画面顺序和表达成立，再估计口播所需停留；估时用于草稿节奏，不是最终声音长度。保留用户已有合理剪辑，不为套用新方案清空重建。现场声按叙事需要保留或调低；此阶段不调用 TTS、不生成替代本人声音的临时配音。
4. **配明确的临时字幕。** 将待录文案按意思分句，在当前草稿内安排可读的临时时间，用 `caption` 操作保存，新增 ID 统一以 `draft-narration-` 开头并保持唯一。面板据此保存 `draftCaptionIds`，后续只替换这个集合；不根据时间或相似文字猜测现有标题的归属。临时字幕来自文字稿的估时，随草稿状态明确标记为待录音对齐；不能称作真实转写、逐词同步或已完成音画同步。保留用户独立标题和字幕，不盲目删除全部文字。
