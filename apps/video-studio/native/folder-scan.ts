...
export const FOLDER_SCAN_LIMITS = Object.freeze({
  maxFiles: 1000,
  maxEntries: 20_000,
  maxDirectories: 2000,
  maxDepth: 16,
...
export interface FolderScanOptions {
  signal?: AbortSignal;
  /** Callers may lower these bounds, never raise the tool's hard limits. */
  limits?: Partial<Record<keyof typeof FOLDER_SCAN_LIMITS, number>>;
  /** Optional read-only progress notification; paths are never exposed here. */
  onProgress?(progress: { entries: number; files: number }): void;
}
...
const sameSnapshot = (a: BigIntStats, b: BigIntStats) =>
  sameIdentity(a, b) && a.size === b.size && a.mtimeNs === b.mtimeNs && a.ctimeNs === b.ctimeNs;
type Observed = { absolute: string; snapshot: BigIntStats };

/** Scan an already authorized directory. Sources remain read-only and all output paths are relative. */
export async function scanFolder(
  ...
  try {
    ...
    for (const key of Object.keys(limits) as (keyof typeof limits)[]) {
      if (
        !(key in FOLDER_SCAN_LIMITS) ||
        !Number.isSafeInteger(limits[key]) ||
        limits[key] < 1 ||
        limits[key] > FOLDER_SCAN_LIMITS[key]
      )
        fail("文件夹扫描限制无效。");
    }
    ...
    const result: FolderScan = { files: [], skipped: 0 };
    let entries = 0,
      directories = 0;
    // Reserve space for JSON punctuation and the bounded skipped count before accumulating rows.
    ...
    const verifyRoot = async () => {
      ...
      if (
        !sameSnapshot(initial, now) ||
        now.isSymbolicLink() ||
        (await realpath(requestedRoot)) !== canonicalRoot
      )
        fail("扫描期间文件夹已变化，请等待文件写入完成后重试。");
    };
    const verify = async (item: Observed) => {
      checkCancelled();
      const now = await lstat(item.absolute, { bigint: true });
      if (
        now.isSymbolicLink() ||
        !sameSnapshot(item.snapshot, now) ||
        (await realpath(item.absolute)) !== item.absolute
      )
        fail("扫描期间素材或目录已变化，请等待文件写入完成后重试。");
    };
    async function visit(absolute: string, relative: string, depth: number, ancestors: Observed[]) {
      await verifyRoot();
      ...
      const directory: Observed = { absolute, snapshot: await lstat(absolute, { bigint: true }) };
      ...
      try {
        for await (const entry of stream) {
          ...
          // Ignore metadata/dependency trees and names that cannot be represented by the resource API.
          if (
            name.startsWith(".") ||
            name === "node_modules" ||
          ...
          if (snapshot.isDirectory()) {
            ...
            await verify(item);
            // Opening without reading checks actual source access. Never follow a replacement symlink.
            ...
            await verify(item);
            const bytes = Number(snapshot.size),
              lastModified = Number(snapshot.mtimeNs / 1_000_000n);
            if (
        ...
        // for-await closes the directory on success and on thrown errors.
        await stream.close().catch((error: NodeJS.ErrnoException) => {
...
/** CLI scans only its authorized working directory; it never accepts arbitrary source paths. */
export async function runFolderScanCli(): Promise<void> {
  try {
