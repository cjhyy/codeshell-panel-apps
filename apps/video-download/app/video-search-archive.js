...
export function cleanSearchRecord(value, normalizeCandidates) {
  ...
  const platforms = [
    ...new Set(value.platforms.filter((item) => ["youtube", "bilibili"].includes(item))),
  ];
  if (!query || !platforms.length) return null;
  const safe = normalizeCandidates(value.candidates, platforms).slice(0, 8);
  const reasons = new Map(
    (Array.isArray(value.candidates) ? value.candidates : []).map((item) => [
      item?.url,
      bounded(item?.reason, 400),
    ]),
  );
  ...
  return {
    id: bounded(value.id, 100) || randomId(),
    query,
    platforms,
    modelId: bounded(value.modelId, 256),
    provider: bounded(value.provider, 120),
    createdAt:
      Number.isFinite(value.createdAt) && value.createdAt > 0 ? value.createdAt : Date.now(),
    status: ["ready", "empty", "error"].includes(value.status) ? value.status : "ready",
    summary: bounded(value.summary, 500),
    candidates: safe.map((candidate) => ({
      ...candidate,
      reason: reasons.get(candidate.url) || candidate.reason,
      evidence: indexed.has(candidate.url) ? "historical-index" : "historical",
    })),
  };
}
