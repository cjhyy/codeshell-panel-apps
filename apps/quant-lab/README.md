...
发现 `automations.createUnique` 时，价格／技术提醒按市场提交固定 key，由 Host
将它限定到当前 Panel、工作区和绑定任务；两个页面同时开启同一市场，兼容 Host
原子保留一条任务。不同任务绑定分别管理，旧无 key 任务仍按已有名称读取，并不
自动合并；删除后允许重新创建，这不是永久请求凭据或执行次数保证。

创建响应丢失不降级到普通创建、不立即重发；显示未确认状态，逐项重试先读取现有
任务，内容相符时直接恢复界面。尚未核对的错误阻止批量开关和旧任务清理，避免把
核对动作变成关闭操作。旧 Host 保留原接口并提示不能保证多页面同时开启不重复。
同 key 内容不同时由 Host 拒绝，后续更新仍需要用户明确操作。
...
Panel 控制器联测，传输／授权适配为夹具；没有启动行情检查或真实通知。
...
这不代表全部控制器已完成跨项目验收，也不改变资讯／市场脉搏创建任务的去重边界。
