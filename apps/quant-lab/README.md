...
自定义条件筛选采用项目存储版本校验。多个页面或设备同时修改时，冲突页面保留
当前条件并停止自动覆盖；可下载筛选草稿，再明确读取最新条件。读取失败、损坏
记录或不兼容版本／字段不再回退后覆盖原记录，草稿下载同时保留读到的原始记录。

保存回执丢失时只读取核对，不重复写入。切换项目会停止尚未发送的旧保存与后续
导出读取，旧请求的迟到结果不会修改新项目界面；已经发出的原项目写入可能完成。
未保存草稿按原项目留在当前页面供下载，关闭页面前需要自行保存备份。旧 Host
继续使用原保存接口，并明确显示不支持多设备版本校验的限制。未公开发布。


待发布（2026-09-24）：回测参数在支持条件保存的 Host 上使用项目版本校验，打开项目不自动重写参数；修改时自动保存有效参数，保留未知顶层字段。冲突、读取失败或无法确认保存时显示状态并停止继续写入，当前草稿可导出 JSON（包括暂未填完的输入），随后读取最新记录或载入当前项目的备份。备份载入作为一次明确的新编辑，仍受当前版本检查约束。旧 Host 保留普通保存并明确提示缺少冲突检查，保存失败不再静默忽略。项目切换不补发旧保存，迟到的参数或策略读取不能覆盖新项目；本页未确认草稿按项目保留，关闭页面前应导出备份。此项不改变回测算法、行情来源或收益假设，也不代表全部投资业务已完成云端验收。
...
价格与技术提醒的关注列表、规则检查结果及事件记录接入同一项目条件存储。旧记录
规范化也使用加载时的版本，不能覆盖另一页面已保存的记录。添加、移除和检查结果
保存失败时，页面保留修改并明确显示“未保存”，不再立即提示成功。提供 JSON 草稿
备份及明确放弃页面修改后读取最新记录；重读失败不清空当前列表。

新建、更新或关闭分市场提醒前，会检查关注记录是否成功保存且版本仍一致；发现
其他设备的修改后停止操作，现有提醒保持原状。用户需读取并核对最新记录后再操作。
各异步步骤绑定项目代次，旧项目的读回、创建结果和失败不能写入新项目的提醒界面，
也不能接着在新项目执行后续步骤。已经发出的 Host 动作不在客户端撤回。

这不是存储与自动化之间的跨资源事务：另一设备仍可能在最后一次版本检查后修改
记录。跨设备完整自动化验收以及实际服务商／物理手机通知，仍需继续完成。
程序定位已改用下述项目 Skill，历史任务不会自动重写。长期个股／板块关注与
这里的“价格与技术提醒”是不同存储，尚未接入本次条件保存。

扩展 `scripts/quant-lab-ui.mjs` 覆盖完整页面中的竞争写入、未保存提示、JSON 备份、
重读失败和恢复、操作提醒前发现外部修改及仅使用最新已保存清单创建任务。可设置
`QUANT_LAB_WATCH_STORAGE_SCREENSHOT` 保存 390px 截图。独立状态机测试覆盖项目切换
时的读取、检查、已发出创建和晚到结果；真实 Node Host 验证新增关注记录版本检查。

### 未发布：提醒创建去重

发现 `automations.createUnique` 时，价格／技术提醒、资讯同步和市场脉搏使用共享
创建逻辑，分别提交 `market-alert.cn/us`、`news-sync.cn/us` 和 `market-pulse.daily`。
Host 将标识限定到当前 Panel、工作区和绑定任务；两个页面同时开启同一任务，兼容
Host 原子保留一条记录。不同任务绑定分别管理，旧无 key 任务仍按已有名称读取，
不自动合并；删除后允许重新创建，这不是永久请求凭据或执行次数保证。

创建响应丢失不降级到普通创建、不立即重发；创建后的核对读取失败或定义不一致
同样要求重新读取。第一次重试只读取现有任务，即使发现其他设备已修改定义，也
不会自动更新或删除；确认不存在或需要更新后，再由用户明确操作。价格提醒尚未
核对的错误阻止批量开关和旧任务清理，避免把核对动作变成关闭操作。旧 Host 保留
原接口，三类任务均提示不能保证多页面同时开启不重复。同 key 内容不同时由 Host
拒绝，不降级创建。

新方法已接通原生桌面与配对 Web：在手机工作台选择同一持久会话后，共用桌面上的
调度器、任务和唯一身份；主机关闭后不能继续执行。主仓库任务分支的 Hub 已组合长期
调度器、共享 Worker 与持久回执，尚未发布到 services 固定依赖。真实提醒执行及手机通知仍待完成。所有写调度文件的进程需使用
兼容 Core；旧版本写入会丢弃新身份字段，不能混用后声称去重有效。

验证覆盖控制器竞争创建、丢失响应与重读、禁止降级、旧 Host 提示；完整浏览器回归
检查 390px 页面发现新方法、创建后响应丢失与单条任务恢复。主仓库新增
`scripts/smoke-quant-unique-automation.mjs <Panel仓库路径>`，真实调度存储与实际
价格提醒、两个资讯市场和市场脉搏控制器联测，使用独立存储实例及真实创建身份，
传输／授权适配为夹具；没有启动行情检查或真实通知。


### 未发布：项目选定版本的程序定位

行情关注、资讯同步与市场脉搏的新任务先调用包内 `quant-lab:project-runtime`
Skill。Host 返回当前项目所选包的实际 Skill 目录，任务从同一个包读取执行程序、
watch 引擎和资讯模块；数据工作目录继续是当前项目。手动资讯同步和市场脉搏也
使用这条路径。Skill、包身份或程序不可用时明确报告 unavailable，不寻找全局
安装版本作为替代。

这项修改需要安装包含新 Skill 的包并更新相应任务。既有持久任务不会被静默重写；
云端任务的来源修订不匹配时，应核对结果并关闭旧任务，再明确新建当前版本任务。
它不迁移研究数据；自动化定义的并发修改由下述条件操作处理。

验证：Quant 离线流程与 154 项测试、完整 Chromium UI 回归、包校验及 Skill 格式
校验通过。浏览器使用受控 Host bridge，没有运行真实行情。主仓库的
包版本测试实际调用 Skill 后运行包内 Node 程序，分别得到旧／新版结果，旧包
缺失时拒绝回退；另用实际 Quant 包在两个临时项目验证三项程序和计算模块可读、
执行目录与所选版本一致。测试版本 0.46.2 只存在临时夹具，没有发布或安装到用户
项目。真实模型是否按说明执行、真实行情服务和手机推送仍须另行验收。


### 未发布：自动化条件更新与删除

兼容 Host 返回任务定义的 `revision`，并声明 `automations.updateIfRevision` /
`automations.deleteIfRevision`。行情关注、资讯、市场脉搏和旧提醒清理优先使用
这些接口，携带读取时的版本；另一页面修改、停用或删除任务后，当前操作被拒绝，
不会降级到旧接口重试。运行次数与运行结果变化不造成编辑冲突。

冲突后保留错误提示与现有任务，下一次点击只重新读取；核对新内容后才能再次
明确更新或删除。后台刷新不会悄悄清除冲突状态。请求响应丢失也不自动改用无条件
写入。未声明新能力的旧 Host 保留兼容行为，不能保证并发编辑不覆盖。

这不是关注记录与自动化的跨资源事务，不会自动迁移来源不同版本的历史任务；
其他 Panel 的任务编辑也需要各自接入。Host 的三个入口已有实际 Electron、配对
HTTP 和云端 HTTP 验证，运行脚本／模型／手机通知的完整业务验收仍单独跟踪。

### 未发布：资讯与市场脉搏的项目切换保护

资讯读取、订阅保存、后台任务变更、通知批次和手动同步现在保留发起时的项目。
切换项目后停止旧流程的后续请求，旧成功／错误／收尾不会覆盖新项目状态。
市场脉搏的旧加载也不会清除新项目正在等待的加载记录。旧资讯卡片不能在新项目
继续打开外链或创建笔记，等待中的外链确认不能覆盖新项目提示。

保护同时检查 Host 工作区代次和控制器 reset，不需要新增 Host 方法或权限。
已经被原项目接受的写入、任务变更和通知不会撤回；切回原项目时应重新读取结果。
这不代表全部控制器已完成跨项目验收；创建去重的能力范围见上文。

`npm test -- --suite quant-lab` 包含每个 Host 请求边界的成功／失败延迟返回检查，
覆盖仅重置、项目切换、新项目已加载、通知批次及外链确认。
完整浏览器回归仍使用 `npm run test:ui:quant-lab`。

### 未发布：长期个股与板块关注的跨设备保存

长期关注使用 Host 声明的 `storage.getSnapshot` / `storage.compareAndSet` 保存。
另一页面或设备已修改时保留当前草稿，停止后续写入与新扫描，提供 JSON 草稿下载和
明确放弃草稿、读取最新记录的入口；读取失败继续保留草稿。响应丢失后先查询，不重发
写入，保存失败不提示关注成功，持仓自动加入关注也不会误报成功。

保存保留兼容记录的顶层及个股／板块扩展字段，明确移除的条目、重点标记和来源不被
旧字段恢复。不支持的版本、无效／重复／超容量列表拒绝写入，避免归一化后静默丢项。
草稿下载仅导出当前页面内容，不声称备份了未能解析的远程原始记录。旧 Host 显示
并发保护限制，继续原有单页面保存方式。

项目切换使旧保存失效；暂停行情扫描不会使项目存储失效，也不会因关注保存而重新
启动扫描。控制器、390px 浏览器冲突／下载／恢复及实际 Node Host 磁盘存储验证通过；
浏览器 Host 为夹具，实际 Host 验证另行进行，不能替代真实手机、服务商或完整提醒验收。
