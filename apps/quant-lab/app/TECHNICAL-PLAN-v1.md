...
状态：S0–S6 完成（v0.4.1）。信息层级重排、盯盘、选股 Spec 2、市场脉搏 Spec 1、多数据源、数据优先首页与运行时降级已落地（v0.12.1）；系统主题推荐、板块内个股、长期关注、时机状态和公告/新闻证据链已落地（v0.13.0）；指数—热点—板块—个股的市场发现首页与全局浅色卡片界面已落地（v0.14.0）；所有 A 股标的入口共用全市场名称目录和无歧义解析（v0.15.0）；通用 Panel App 本地数据目录与行情/选股快照恢复已落地（v0.16.0）；名称/代码搜索先打开确定性个股行情、180 日趋势与公告新闻，Agent 诊断降为详情二级动作（v0.17.0）；个股诊断在详情内持续等待唯一结果路径并直接回显摘要、事实、研究项、风险和来源（v0.17.1）；个股行情与诊断迁入独立顶层页面，所有股票入口统一跳转（v0.18.0）；盘中实时态与复盘态分离，收盘快照恢复后不再被自动覆盖，最后查看板块可跨重开恢复（v0.18.1）；板块代表股、等待买点与完整收盘时机确认分层，盘中信号不进入确认区（v0.19.0）；指数—热点—板块—个股连续浏览、五类具名策略、A 股执行约束历史校准与预测快照复盘闭环已落地（v0.20.0）；板块实时详情、独立研究优先股和先结论后证据的个股浏览已落地（v0.21.0）；首页机会、实时行情、研究复盘分区和按需展开选股漏斗已落地（v0.22.0）；个股公司研究入口、业绩/估值/行业/公告分层报告与生成状态回显已落地（v0.23.0）；长期关注与条件提醒统一在顶层关注页分层展示已落地（v0.24.0）；可核验的行情更新状态、数据新鲜度、刷新倒计时、数据源覆盖和本轮变化已落地（v0.25.0）；120/300 只 A 股三年前复权历史基础库、可续跑初始化、复权基准重建、智能增量更新及个股/选股缓存复用已落地（v0.26.0）；双线趋势、KDJ、量能结构和价格形态四维证据、异常量风险提示及其对板块内研究排序的受限参与已落地（v0.27.0）；初始化历史数据源显式选择、来源清单固化及增量更新防混源已落地（v0.28.0）；腾讯/东方财富/Tushare 三源、初始化可用性探测、自动优先级降级和失败原因记录已落地（v0.29.0）；KHunter 式“数据准备—市场更新—执行选股—关注跟踪”驾驶舱及初始化后直接进入选股的主流程已落地（v0.30.0）；东方财富/新浪财经/证监会/人民银行/巨潮资讯的多源新闻分层、同事件聚合与官方披露优先已落地（v0.31.0）；美股名称/ticker 搜索、Yahoo Finance 行情与复权趋势、本地快照，以及 SEC/新闻/公开社媒分层的按需 Deep Research 已落地（v0.32.0）；全部在市 A 股三年前复权历史初始化、长任务进度及断点续跑已落地（v0.33.0）；已有 300 只基础库的初始化档位可重新选择（v0.33.1）；等待股三条去重未通过原因和全市场增量更新说明已落地（v0.34.0）；基于已校验价格锚点、用户周期/风险/仓位上限的条件策略草案已落地（v0.38.0）；全局一键选股入口及准备/执行/查看状态路由已落地（v0.38.1）；进入即显示的研究中心、已有结果/下一步研究分层及 Agent/直接数据入口标识已落地（v0.39.0）；面板侧 Host 调用削峰、交互额度预留、发现结果复用与对应功能懒加载已落地（v0.39.1）；研究复盘候选卡复用本地固定选股且不再创建 Agent 任务已落地（v0.39.2）；选股结果/市场看板/研究复盘三工作区、五档强弱与六阶段、25 套策略、因子证据、公开社媒雷达、白底个股报告和直接单股回测已落地（v0.43.0）；轻量自动补齐、次日与高开复盘、关注筛选和带指标的回测结果索引已落地（v0.43.1）；跨页签自动补齐、并发状态合并、日期优先鲜度、项目 CSV 顺序直更和当前研究池自定义条件筛选已落地（v0.44.1）。
...
因此选股功能的诚实定位是:**可做"今天筛出候选",但历史回测的绝对收益不可信,只有相对排序有参考价值**。这条限制像复权口径一样写死进报告。

首版实现为冻结的两阶段 `cn-trend-volume-v1`，不让 Agent 参与核心排名：

- 新浪当前沪深 A 股快照建立当日 universe，固定排除 ST/退市风险名、低价、低成交额、低流通市值与极端换手；覆盖低于 4,000 只整次失败。
- 基础池按流动性为主预排，最多 120 只以单路节流方式读取腾讯 120 日前复权日线；历史覆盖低于 80% 不写报告，缺数标的零填充禁止。规模上限以真实接口稳定性为准，不用高并发换取虚假的“全覆盖”。
- 排名使用 20/60 日趋势、距 60 日高点、量比、成交额与换手质量；均线未确认、量能不足、远离高点、接近涨停追高或偏离 20 日线过远均 fail closed。
- 输出沿用严格 `data/market-insights/*-candidates.json` 契约，强制展示 universe/coverage/通过数、实际值、风险与高分排除反例；盘中标为 provisional，收盘后须重跑。
- 旧版 `buildMarketInsightTask("candidates")` 与候选 JSON 解析仅为历史快报兼容保留；当前界面统一直接运行选股页的固定本地程序，`submitMarketCommand("candidates")` 也强制转入该路径，不再提交 Agent。旧的 LLM 自由选股 prompt 已移除。

该实现解决"今天如何稳定缩小研究范围"，没有解决 point-in-time universe、退市链、行业中性、公告/财务核验或策略组合收益验证；这些仍是后续边界，不得把相对得分改称预测概率或买入评级。
...
## 19. 行情 Spec 1：每日市场脉搏（v0.9.0）

- 固定工具 `build-market-pulse.mjs` 复算沪深有效股票宽度、成交额、中位涨跌与板块阈值近似涨跌停；
  全市场覆盖不足 fail closed，不让 Agent 自行补数。
- 上证、深证、创业板和沪深 300 只读取少量 250 日指数历史，以 MA20/60/120、20/60/120 日收益和
  250 日回撤判定长期阶段；不使用大批量个股历史，降低限流与复权风险。
- 新浪行业快照提供成分平均涨跌与领涨股，不冒充交易所行业指数或资金流；东方财富 7×24 只做
  24 小时关键词关联，不输出新闻情绪和价格因果。板块/新闻单源失败显式降级。
- 行情页提供独立 opt-in 自动化 `投资工作台 · A股市场脉搏`，工作日 10:10/15:10 运行；创建、更新、
  删除均以 Host list 二次核验。盘中报告标 provisional，收盘后需要重跑；运行日没有新交易日快照时
  标「最近收盘」，不把周末、节假日或源延迟的旧行情冒充当天行情。

## 20. 多数据源历史行情（v0.10.0）

- 新增浏览器/Node 共用的 `market-data-sources.mjs` 注册表，冻结来源 id、市场、复权能力和凭据环境变量名；
  腾讯、Yahoo、Tushare Pro、Alpha Vantage 与 Massive 都通过同一选择和元数据契约。
- 历史数据中心允许按市场选择 `auto` 或明确供应商，并动态收敛复权选项。`auto` 只会尝试口径兼容且
  凭据已配置的备用源；显式选择不静默降级。既有数据更新固定使用 sidecar 记录的原供应商，供应商或
  复权口径变化均 fail closed，避免一个 CSV 混入不同厂商数据。
- Tushare 使用 HTTPS POST `daily + adj_factor`，按官方公式计算 qfq/hfq，成交量从手归一到股；
  Alpha Vantage 用 adjusted-close 比例同步缩放 OHLC；Massive 用日级 aggregate 的 `adjusted` 开关。
  三者只从 `TUSHARE_TOKEN`、`ALPHAVANTAGE_API_KEY`、`MASSIVE_API_KEY` 读取凭据，不进入 prompt、命令、
  sidecar 或错误摘要。
- 所有远端访问固定 HTTPS origin、拒绝重定向、限制响应体与超时；来源响应仍经过统一 OHLC 正值、
  高低价关系、重复日期和最少 K 线门禁。离线测试覆盖来源能力、缺凭据、复权换算、显式来源、自动
  降级和旧数据源锁定；浏览器 e2e 覆盖来源/复权联动与任务文本零凭据。

## 21. 自动行情层与可选诊断（v0.11.0）

- 首页进入后直接读取主要指数、全市场宽度、成交额、涨跌停近似与行业强弱；不依赖持仓、项目快报或
  Agent 诊断。盘中可见时每 3 分钟刷新，休市每 15 分钟刷新，面板隐藏或离开行情模块时停止计时器。
- Panel CSP 保持零网络。新增经安装审核的 `process` 权限，只解析 PATH 中的 Node.js，使用 Downloads
  目录 opaque handle 作为受限 cwd，并以独立参数运行安装快照内固定工具的 `--stdout` 模式；无 shell、
  无工作区写入、无用户参数拼接、无凭据环境变量。首次执行仍由 Host 按 app/revision/executable 确认。
- stdout envelope 与内层 market-overview 分别严格校验：版本、类型、ISO 时点、交易阶段、指数白名单、
  行业标识、数值范围、涨跌家数守恒、输出大小和未来时点任一失败即保留旧快照。45 秒或 1 MiB 页面
  上限触发取消；Host 另有进程生命周期和 4 MiB 总输出上限。
- 页面明确区分盘中累计、当日收盘和最近收盘，并显示行情/本机刷新时点、来源与降级状态。自动行情
  是公开源准实时快照，不声称交易所逐笔或 Level-2。新浪/腾讯/东方财富任一辅助源失败不会被解释为
  “没有热点”；核心全市场覆盖不足则整次拒绝。
- 原“大盘诊断”改为可选“盘面解读与留档”；实时事实已经直接显示，只有用户想保存研究记录时才
  提交 Agent。10:10/15:10 automation 也改称每日留档，和页面自动刷新互不依赖。
- `scripts/quant-lab-market-pulse.mjs` 覆盖 live envelope 防篡改与刷新策略；浏览器 e2e 模拟完整
  `process.find → getKnownDirectory → spawn → output/exit`，验证自动启动、四指数、5,208 只宽度、成交额、
  板块和三分钟文案。

## 22. 数据优先的行情首页（v0.12.0）

- 首页快捷入口改为页内定位，不再提交 Agent；缺持仓时仍直接展示行情。
- 自动快照增加涨幅榜、跌幅榜、成交活跃榜、盘中关注池、最新快讯与最近完整交易日龙虎榜。
  关注池使用冻结的当日价格/成交条件排序，只说明入选原因与风险，不标记买入推荐。
- 新增数据均进入同一严格校验边界：代码、数值、时点、外链、龙虎榜符号和重复项异常时拒绝更新。
- 已保存研究、自动留档、指数诊断、市场雷达与候选扫描收入默认关闭的二级区域，不再占据首屏。
- 浏览器 e2e 验证榜单切换、快讯、龙虎榜、关注卡、页内定位与研究区默认收起；既有 Agent 与自动化流程仍在展开后继续通过。

## 23. 跨页签、跨窗口与项目 CSV 自动补齐（v0.44.1）

- 历史库检查从 Today 页生命周期中拆出；恢复到研究、关注或其他已保存页签时，仍会先读取共享本地清单，必要时启动 `--incremental-only` 固定程序。
- 面板在隐藏状态启动、随后于新一天恢复可见且仍停留在非行情页时，会执行一次 `refreshOnce` 行情日期探测；该调用不改变市场页 active 状态，因此完成后不会留下隐藏轮询。只有网络核验成功才记录当天已探测，本地旧快照不会误占防重复标记。
- 应用持续可见但停在非行情页时，按北京时间工作日 15:12 安排一次轻量收盘探测；若上游已返回当日快照但仍标 provisional，则每 15 分钟复核一次并在 17:00 截止。周末不安排定时探测，节假日即使工作日触发也只会得到最近收盘，不会写入虚构交易日。
- 行情刷新和页面初始化同时触发时共享一个 status promise；只有已确认需要启动的补齐任务才写入五分钟防重复窗口。
- 全市场清单只统计当前在市股票，退市或更名遗留序列保留在磁盘但不再虚增覆盖数；Host 桥接只回传日期分布而不回传五千多条记录。
- 本地状态同时回传最近五个完整收盘快照的日期与股票数；界面把“快照已留存”与“每只股票最后一根 K 线”分开，停牌股缺少当日成交不会被误判为整个交易日未补。
- 其他窗口启动的任务使用 15 秒低频 status 轮询；任务结束后自动撤销“补齐中”、显示最终覆盖并提醒重新执行今日选股。轮询只读本地状态，不请求行情、不启动 Agent。
- 共享基础库核对完成后，按原来源和复权口径顺序更新项目内落后的 A 股单股 CSV；卡片显示排队、核对日期与最新 K 线日期，供应商已核对但停牌无新 K 线时不重复联网。工作区切换会取消尚未写回的旧项目更新，避免跨项目污染。
