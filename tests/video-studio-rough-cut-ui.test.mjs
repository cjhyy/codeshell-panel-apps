...
before(async () => {
  ...
  assert.equal(generated.status, 0, `FFmpeg must create actual playable source media: ${generated.stderr}`);
  ...
  server = createServer(async (request, response) => {
    ...
    try {
      ...
      const end = range && range[2] ? Math.min(bytes.length - 1, Number(range[2])) : bytes.length - 1;
      const body = bytes.subarray(start, end + 1);
      response.writeHead(range ? 206 : 200, {
        "Content-Type": { ".html": "text/html", ".css": "text/css", ".mjs": "text/javascript", ".mp3": "audio/mpeg" }[extname(path)] ?? "application/octet-stream",
        "Content-Length": body.length,
        "Accept-Ranges": "bytes",
        ...(range ? { "Content-Range": `bytes ${start}-${end}/${bytes.length}` } : {}),
        "Cache-Control": "no-store",
        "Content-Security-Policy": "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; media-src 'self' blob:; font-src 'self' data:; connect-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'",
      });
      response.end(body);
    } catch {
...
const saved = (page) => page.waitForFunction(() => document.querySelector("#save-state")?.textContent === "已自动保存");

async function importedPage() {
  const page = await openPage();
  await page.locator("#media-input").setInputFiles([
    sourcePath,
    resolve(root, "tests/fixtures/static-tone.wav"),
  ]);
  await page.waitForFunction(() => window.__roughCutTools.read_video_project().project.assets.length === 2);
  await saved(page);
...
async function seekSource(page, frame) {
  ...
  await page.waitForFunction((value) =>
    document.querySelector("[data-roughcut-scrub]")?.value === String(value), frame);
...
test("real source preview, I/O marks, invert/undo, ordered cuts and portable project survive a complete workflow", { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => Number(document.querySelector("[data-roughcut-scrub]")?.value) > 35);
    await page.locator('[data-action="roughcut-play"]').click();
    assert.deepEqual((await state(page)).project, before.project, "Source playback does not edit the composition");
    assert.equal((await state(page)).playheadFrame, before.playheadFrame, "Source playback has its own playhead");
    ...
    await page.locator('[data-action="roughcut-play"]').click();
    await page.waitForFunction(() => Number(document.querySelector("[data-roughcut-scrub]")?.value) > 0);
    assert.deepEqual((await state(page)).project, before.project, "Previewing the real WAV does not add an audio track");
    assert.equal((await state(page)).playheadFrame, before.playheadFrame);
    ...
    assert.equal(await page.locator("#roughcut-out").inputValue(), "00:00:02:00", "O includes the current source frame");
    assert.deepEqual((await state(page)).project, before.project, "Unsaved I/O marks are only a draft");
    ...
    assert.deepEqual(marked.project.roughCuts.map(({ inFrame, outFrame, name }) => [inFrame, outFrame, name]), [
      [30, 60, '开场, "重点"'], [90, 120, "结尾"],
    ]);
    ...
    await saved(page);
    assert.deepEqual((await state(page)).project.roughCuts.map(({ inFrame, outFrame }) => [inFrame, outFrame]), [
      [0, 30], [60, 90], [120, 180],
    ]);
    await page.locator('.source-history [data-action="undo"]').click();
    await saved(page);
    assert.deepEqual((await state(page)).project.roughCuts, marked.project.roughCuts, "Undo restores the original marker IDs, names and ranges");
    ...
    assert.deepEqual(ordered.roughCuts.map(({ id }) => id), [second.id, first.id]);
    assert.deepEqual(await page.locator("[data-roughcut-row]").evaluateAll((rows) => rows.map((row) => row.dataset.roughcutRow)), [second.id, first.id]);
    ...
    assert.deepEqual((await state(page)).project, ordered, "Editing a name cannot trigger marker, split or composition deletion shortcuts");
    await page.locator("#roughcut-in").fill("1");
    await page.locator("#roughcut-in").press("Backspace");
    assert.deepEqual((await state(page)).project, ordered, "Deleting an input value does not delete the selected composition clip");
    ...
    assert.deepEqual(assembled.clips.slice(1).map(({ assetId, inFrame, outFrame }) => [assetId, inFrame, outFrame]), [
      [video.id, 90, 120], [video.id, 30, 60],
    ...
    await page.waitForFunction(() => window.__roughCutTools.read_video_project().project.assets.length === 0);
    await page.locator("#project-input").setInputFiles({ name: json.name, mimeType: "application/json", buffer: json.bytes });
    await saved(page);
    assert.deepEqual((await state(page)).project, assembled, "Opening the downloaded JSON keeps markers and assembled clips intact");
    await page.locator("#media-input").setInputFiles([sourcePath, resolve(root, "tests/fixtures/static-tone.wav")]);
    await page.waitForFunction(() => window.__roughCutTools.read_video_project().missingAssetIds.length === 0);
    assert.equal((await state(page)).project.assets.length, 2, "Reconnecting source files does not duplicate saved assets");
    await page.locator(`[data-rough-source="${video.id}"]`).click();
    ...
    await seekSource(page, 40);
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true, "The 640px editor must not overflow horizontally");
    for (const selector of ["[data-roughcut-panel]", "#roughcut-source", ".roughcut-range-editor", ".roughcut-batch"]) {
      const bounds = await page.locator(selector).boundingBox();
      assert.ok(bounds && bounds.x >= 0 && bounds.x + bounds.width <= 640.5, `${selector} remains within the narrow viewport`);
    }
...
test("audio source marks append to independent audio without changing the retained video", { timeout: 45_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(project.audioClips.map(({ assetId, inFrame, outFrame, startFrame }) => [assetId, inFrame, outFrame, startFrame]), [
      [audio.id, 0, audio.durationFrames, 0],
    ]);
    await page.locator('.source-history [data-action="undo"]').click();
    await saved(page);
    assert.equal((await state(page)).project.audioClips?.length ?? 0, 0);
    assert.equal((await state(page)).project.roughCuts.length, 1, "Undoing insertion preserves the reusable source mark");
  } finally {
    await page.close();
  }
});
